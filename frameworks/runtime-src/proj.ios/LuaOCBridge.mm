...
+ (void) showSMSView:(NSDictionary*)dict
{
	...
	[[LuaOCBridge getRoomViewController] showSMSView:content smsto:to];
}
...
//	[dic setValue:@"" forKey:@"deviceId"];
//	NSString* networkType = [LuaOCBridge getfNetworkType];
//	networkType = (networkType == nil || networkType == NULL)?@"unknow":networkType;
//	[dic setValue:networkType forKey:@"networkType"];
...
{
	NSString* content = [dict objectForKey:@"content"];
	if (content == nil || content == NULL || [content isEqualToString:@""]) {
		return;
	}
