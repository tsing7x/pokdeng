...
public class BluePayPlugin extends LifecycleObserverAdapter implements IPlugin {
	...
	class PayCallback extends IPayCallback {
		 ...
		 */
		@Override
		public void onFinished(int code, BlueMessage blueMsg) {
			String title = "";
			isPurchasing = false;
			String result = null;
			int bcode = 0;
			...
			dataMap.put("bcode", String.valueOf(bcode));
...
//			Log.e("payBySMS","===========================================");
	...
	public void payByCashcard(String uid,String pid, String transactionId, String propsName, String publisher, String cardNo, String serialNo){
		if(publisher == "12Call"){
			publisher = PublisherCode.PUBLISHER_12CALL;	
		}else if(publisher == "trueMoney"){
			publisher = PublisherCode.PUBLISHER_TRUEMONEY;
		}
		this.uid = uid;
		if(isSetupComplete && isSupported && !isPurchasing){
			BluePay.getInstance().payByCashcard(this.mActivity,uid,transactionId, propsName,publisher, cardNo, serialNo,mPayCallback);
			isPurchasing = true;
		}
