package com.boyaa.cocoslib.iab;
...
import com.boyaa.cocoslib.iab.util.IabHelper;
import com.boyaa.cocoslib.iab.util.IabResult;
import com.boyaa.cocoslib.iab.util.Inventory;
import com.boyaa.cocoslib.iab.util.Purchase;
import com.boyaa.cocoslib.iab.util.SkuDetails;
...
public class InAppBillingPlugin extends LifecycleObserverAdapter implements IPlugin {
		...
		public void onConsumeFinished(Purchase purchase, IabResult result) {
			...
			if (result.isSuccess()) {
				...
				for(int i = 0; i < 4; i++) {//重试4次
					...
					} catch (com.boyaa.cocoslib.iab.util.IabException e) {
						Log.e(TAG, e.getMessage(), e);
					}
				}
			}
	...
	public void makePurchase(String orderId, String sku, String uid, String channel) {
		if(helper != null && isSetupComplete && isSupported && !isPurchasing) {
			...
			params.add(new BasicNameValuePair("pid", sku));
			params.add(new BasicNameValuePair("channel", channel));
			Log.e(TAG, "makePurchase-- uid: " + uid + " orderId:"  +orderId + " pid:" +  sku + " channel:" +channel);
			
			
			//boyaaPay extraData:orderId
			helper.launchPurchaseFlow(Cocos2dxActivityWrapper.getContext(), sku, REQUEST_CODE, purchaseFinishedListener, orderId);
			
			
			
//			helper.launchPurchaseFlow(Cocos2dxActivityWrapper.getContext(), sku, REQUEST_CODE, purchaseFinishedListener, URLEncodedUtils.format(params, "UTF-8"));
			isPurchasing = true;
			purchasingSku = sku;
		}
	}
