...
function SocketService:ctor(name, protocol)
    ...
    self.parser_ = PacketParser.new(protocol.CONFIG.SERVER, self.name_)
...
function SocketService:createPacketBuilder(cmd)
    return PacketBuilder.new(cmd, self.protocol_.CONFIG.CLIENT[cmd], self.name_)
end
...
function SocketService:connect(host, port, retryConnectWhenFailure,ipv6)
    self:disconnect()
    if not self.socket_ then
        SOCKET_ID = SOCKET_ID + 1
        self.socket_ = cc.net.SocketTCP.new(host, port, retryConnectWhenFailure or false,ipv6)
...
function SocketService:setOption(opt,value)
