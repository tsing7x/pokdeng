...
function ScrollView:setScrollContent(content)
    if self.scrollContent_ then
        if self.scrollContent_ == content then
            self:resetScrollContentCascadeBoundingBox()
    ...
    -- local casRect = self.scrollContent_:getCascadeBoundingBox() or cc.rect()
    local casRect = self:getScrollContentCascadeBoundingBox() or cc.rect()
    ...
    -- self.scrollContent_:addNodeEventListener(cc.NODE_TOUCH_EVENT, handler(self, self.onTouch_))
...
function ScrollView:setScrollBar(scrollBar)
    ...
    if self.direction_ == ScrollView.DIRECTION_VERTICAL then
        if self.scrollLength_ and self.scrollLength_ > 0 then
            -- 计算滚动条尺寸级滚动范围
...
function ScrollView:setScrollContentTouchRect()
    ...
    -- self.scrollContent_:setCascadeBoundingBox(cc.rect(self.viewRectOriginPoint_.x, self.viewRectOriginPoint_.y, self.viewRect_.width, self.viewRect_.height))
    self:setScrollContentCascadeBoundingBox(cc.rect(self.viewRectOriginPoint_.x, self.viewRectOriginPoint_.y, self.viewRect_.width, self.viewRect_.height))
end

function ScrollView:setScrollContentCascadeBoundingBox(rect)
    -- body
    self.scrollContent_.mBoundingBox_ = rect
end

function ScrollView:resetScrollContentCascadeBoundingBox()
    self.scrollContent_.mBoundingBox_ = nil
end

function ScrollView:getScrollContentCascadeBoundingBox()
    if self.scrollContent_.mBoundingBox_ then
        return self.scrollContent_.mBoundingBox_
    end
