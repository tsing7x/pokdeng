...
function PageListItem:setData(data,startIndex,itemClass)
    ...
    for i=1,count do
        ...
        if not data[startIndex+(i-1)] and item then
...
function PageListItem:updateData(index,data)
    if self.itemList_ and self.itemList_[index] then
        self.itemList_[index]:setData(data)
    end
end
