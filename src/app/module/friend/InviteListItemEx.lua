...
local InviteListItemEx = class("InviteListItemEx", function ()
    return display.newNode()
end)

InviteListItemEx.ITEM_WIDTH = 238
InviteListItemEx.ITEM_HEIGHT = 90

function InviteListItemEx:ctor(data, owner, popup, index)
    self:setNodeEventEnabled(true)
    self:setContentSize(218, 70)
...
function InviteListItemEx:createContent_()
    ...
    -- nick
    display.newTTFLabel({text = nk.Native:getFixedWidthText("", 20, data.name or "", 110), color = styles.FONT_COLOR.DARK_TEXT, size = 20,
        align = ui.TEXT_ALIGN_LEFT, dimensions=cc.size(112, 0)})
...
function InviteListItemEx:lazyCreateContent()
...
function InviteListItemEx:onItemDeactived()
...
function InviteListItemEx:onAvatarLoadComplete_(success, sprite)
    if success then
...
function InviteListItemEx:onBgTouch_(target, evtName)
...
function InviteListItemEx:getData()
    return self.data_
end

function InviteListItemEx:isSelected()
    return self.isSelected_
end

function InviteListItemEx:setSelected(isSelected)
...
function InviteListItemEx:onCleanup()
    nk.ImageLoader:cancelJobByLoaderId(self.userAvatarLoaderId_)
end

return InviteListItemEx