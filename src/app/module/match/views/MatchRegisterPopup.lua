...
-- 用户报名弹窗 -- 新版定时赛
local MatchRegisterPopup = class("MatchRegisterPopup", nk.ui.Panel)
local WIDTH = 695
local HEIGHT = 425
local TOP_HEIGHT = 55

function MatchRegisterPopup:ctor(matchData,callback)
    -- dump(matchData,"MatchRegisterPopup:ctor")
...
function MatchRegisterPopup:createNode_()
...
function MatchRegisterPopup:show()
	self:showPanel_()
end
function MatchRegisterPopup:onShowed()
...
function MatchRegisterPopup:checkFitEntry()
...
function MatchRegisterPopup:onMainTabChange_(selectedTab)
...
function MatchRegisterPopup:hide()
	self:hidePanel_()
end
function MatchRegisterPopup:onCleanup()
    nk.http.cancel(self.registerRequestId_)
end
function MatchRegisterPopup:loadImageTexture(data)
...
function MatchRegisterPopup:onAvatarLoadComplete(success, sprite)
    -- do return end
    if success then
        self.icon1_:show()
...
-- 7102 初级赛门票
-- 7104 通用门票
function MatchRegisterPopup:checkEntryCondition(id)
...
function MatchRegisterPopup:onRegisterHandler()
