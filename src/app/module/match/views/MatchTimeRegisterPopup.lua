...
-- 用户报名弹窗 -- 新版定时赛
local MatchTimeRegisterPopup = class("MatchTimeRegisterPopup", nk.ui.Panel)
local WIDTH = 695
local HEIGHT = 425
local TOP_HEIGHT = 55

function MatchTimeRegisterPopup:ctor(matchData, callback)
    -- dump(matchData,"MatchTimeRegisterPopup:ctor")
...
function MatchTimeRegisterPopup:createNode_()
...
function MatchTimeRegisterPopup:show()
	self:showPanel_()
end
function MatchTimeRegisterPopup:onShowed()
...
function MatchTimeRegisterPopup:checkFitEntry()
...
function MatchTimeRegisterPopup:onMainTabChange_(selectedTab)
...
function MatchTimeRegisterPopup:hide()
	self:hidePanel_()
end
function MatchTimeRegisterPopup:onCleanup()
    nk.http.cancel(self.registerRequestId_)
end
function MatchTimeRegisterPopup:loadImageTexture(data)
...
function MatchTimeRegisterPopup:onAvatarLoadComplete(success, sprite)
    -- do return end
    if success then
        self.icon1_:show()
...
-- 7102 初级赛门票
-- 7104 通用门票
function MatchTimeRegisterPopup:checkEntryCondition(id)
...
function MatchTimeRegisterPopup:onRegisterHandler()
    ...
    self.registerRequestId_ = nk.http.registerMatch(id,mtype,ticketId,timestamp,
        function(data)
            ...
            dump("id:" .. (id or 0) .. " matchid:" .. (matchid or "") .. " matchTime:" .. (matchTime or 0))
