...
local MatchYouWinAnim = class("MatchYouWinAnim", function ()
    return display.newNode()
end)

function MatchYouWinAnim:ctor()
    self.lightBatchNode_ = display.newBatchNode("room_texture.png"):addTo(self)
    self.batchNode_ = display.newBatchNode("room_texture.png"):addTo(self)
...
function MatchYouWinAnim:onEnter()
    -- 添加至舞台开始动画
...
function MatchYouWinAnim:onPlay()
     self:scale(0.2):scaleTo(0.5, 1)
    self.lightBatchNode_:runAction(cc.RepeatForever:create(cc.RotateBy:create(6, 360)))
    for i = 1, 4 do
        self.flashs_[i]:runAction(cc.RepeatForever:create(transition.sequence({cc.ScaleTo:create(0, 0.9, 0.9), cc.ScaleTo:create(0.15, 1.1, 1.1), cc.ScaleTo:create(0.15, 0.9, 0.9)})))
...
function MatchYouWinAnim:onExit()
    -- 从舞台移除停止动画
...
function MatchYouWinAnim:dispose()

end

return MatchYouWinAnim