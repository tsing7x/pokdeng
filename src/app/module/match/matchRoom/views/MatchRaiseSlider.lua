...
local MatchRaiseSlider = class("MatchRaiseSlider", function()
    return display.newNode()
end)

function MatchRaiseSlider:ctor()
    ...
    MatchRaiseSlider.THUMB_BOUND_TOP = trackY + 218 - 176 / 2
    MatchRaiseSlider.THUMB_BOUND_BOTTOM = trackY - 218 + 176 / 2
    MatchRaiseSlider.THUMB_BOUND_HEIGHT = MatchRaiseSlider.THUMB_BOUND_TOP - MatchRaiseSlider.THUMB_BOUND_BOTTOM
...
function MatchRaiseSlider:showPanel()
    self:setSliderPercentValue(0)
    return self:show()
end

function MatchRaiseSlider:setButtonStatus(allPotEnabled, q3PotEnabled, halfPotEnabled, tripleEnabled, isMaxAllin)
...
function MatchRaiseSlider:hidePanel()
    self:setSliderPercentValue(0)
    return self:hide()
end

function MatchRaiseSlider:onButtonClicked(callback)
    self.buttonClickedCallback_ = callback
    return self
end

function MatchRaiseSlider:setValueRange(valueMin, valueMax)
...
function MatchRaiseSlider:setValue(val)
...
function MatchRaiseSlider:getValue()
    return math.round(self:getSliderPercentValue() * self.valueRange_ + self.valueMin_)
end

function MatchRaiseSlider:setSliderPercentValue(newVal)
    assert(newVal >= 0 and newVal <= 1, "slider value must be between 0 and 1")
    self:onSliderPercentValueChanged_(newVal, true)
    self.thumb_:setPositionY(MatchRaiseSlider.THUMB_BOUND_BOTTOM + MatchRaiseSlider.THUMB_BOUND_HEIGHT * newVal)
    return self
end

function MatchRaiseSlider:getSliderPercentValue()
    return (self.thumb_:getPositionY() - MatchRaiseSlider.THUMB_BOUND_BOTTOM) / MatchRaiseSlider.THUMB_BOUND_HEIGHT
end

function MatchRaiseSlider:onSliderPercentValueChanged_(newVal, forceUpdate, needSound)
...
function MatchRaiseSlider:onThumbTouch_(evt)
    ...
    elseif name == "moved" then
        ...
        if toY >= MatchRaiseSlider.THUMB_BOUND_TOP then
            toY = MatchRaiseSlider.THUMB_BOUND_TOP
        elseif toY <= MatchRaiseSlider.THUMB_BOUND_BOTTOM then
            toY = MatchRaiseSlider.THUMB_BOUND_BOTTOM
        end
        self.thumb_:setPositionY(toY)
        local val = (toY - MatchRaiseSlider.THUMB_BOUND_BOTTOM) / MatchRaiseSlider.THUMB_BOUND_HEIGHT
...
function MatchRaiseSlider:onButtonClicked_(tag)
