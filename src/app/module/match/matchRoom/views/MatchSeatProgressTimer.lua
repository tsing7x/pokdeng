...
-- Date: 2016-12-19 18:45:18
-- Copyright: Copyright (c) 2015, BOYAA INTERACTIVE CO., LTD All rights reserved.
-- Description: SeatProgressTimer Class Modified By TsingZhang.
--
...
local MatchSeatProgressTimer = class("MatchSeatProgressTimer",function()
    return display.newNode()
end)

function MatchSeatProgressTimer:ctor(countdown)
    self:setNodeEventEnabled(true)
    setCountdown(countdown - 1)
    self:addNodeEventListener(cc.NODE_ENTER_FRAME_EVENT, handler(self, self.onEnterFrame))
end

function MatchSeatProgressTimer:onEnter()
    -- 添加至舞台开始渲染
    self.frameCount_ = 0
...
function MatchSeatProgressTimer:onEnterFrame(evt, isFastForward)
...
function MatchSeatProgressTimer:remove()
    self:unscheduleUpdate()
    self:removeFromParent()
end

return MatchSeatProgressTimer