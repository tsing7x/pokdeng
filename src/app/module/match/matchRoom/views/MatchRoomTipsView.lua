...
--
local MatchRoomTipsView = class("MatchRoomTipsView", function() return display.newNode() end)

MatchRoomTipsView.WIDTH = display.width * 0.7 - 16

local TIPS = bm.LangUtil.getText("ROOM", "TIPS")
local INTERVAL = 30

function MatchRoomTipsView:ctor()
    self.background_ = display.newScale9Sprite("#room_tips_background.png", 0, 0, cc.size(MatchRoomTipsView.WIDTH, 72)):addTo(self)
    ...
    local w, h = MatchRoomTipsView.WIDTH - 4, 72 - 4
    ...
    self.label_ = display.newTTFLabel({size=24, color=cc.c3b(0x2b, 0x56, 0x86),
            text=TIPS[self.currentIndex_],
            dimensions=cc.size(MatchRoomTipsView.WIDTH - 8, 72),
            align=ui.TEXT_ALIGN_CENTER})
        :addTo(self.clip_)
end

function MatchRoomTipsView:play()
...
function MatchRoomTipsView:stop()
...
function MatchRoomTipsView:playNext_()
    if self.isPlaying_ then
        self.currentIndex_ = self.currentIndex_ + 1
        if self.currentIndex_ > #TIPS then
            self.currentIndex_ = 1
        end
        local nextLabel = display.newTTFLabel({size=24, color=cc.c3b(0x2b, 0x56, 0x86),
                text=TIPS[self.currentIndex_],
                dimensions=cc.size(MatchRoomTipsView.WIDTH - 8, 72),
                align=ui.TEXT_ALIGN_CENTER})
            :pos(0, -72)
...
function MatchRoomTipsView:randomIndexArr_(total)
    local src = {}
    for i = 1, total do
        src[i] = i
    end
