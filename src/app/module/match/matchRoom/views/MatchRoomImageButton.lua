...
--
local MatchRoomImageButton = {}

local function exportMethods(target)
    for k, v in pairs(MatchRoomImageButton) do
        if k ~= "create" then
            target[k] = v
        end
    end
...
function MatchRoomImageButton.create(filename)
    return exportMethods(display.newSprite(filename, x, y, params)):ctor()
end

function MatchRoomImageButton:ctor()
...
function MatchRoomImageButton:onClick(handler)
    self.clickHandler_ = handler
    return self
end

function MatchRoomImageButton:onHelperTouch_(evt, isTouchInSprite)
...
function MatchRoomImageButton:flash(times)
    times = times or 10
    if times == -1 then
...
function MatchRoomImageButton:stopFlash()
    self.isFlashing_ = false
    self:stopAllActions()
    self:setOpacity(128)
    return self
