...
local MatchWinCardAnim = class("MatchWinCardAnim", function ()
    return display.newNode()
end)


MatchWinCardAnim.s_nodeW = 330

MatchWinCardAnim.s_nodeH = 110




function MatchWinCardAnim:ctor()
    -- 打开node event
    self:setNodeEventEnabled(true)
end


function MatchWinCardAnim:show(datas)


    if not self.bg_ then
        self.bg_ = display.newScale9Sprite("#panel_bg.png",0,0,cc.size(MatchWinCardAnim.s_nodeW,MatchWinCardAnim.s_nodeH))
...
function MatchWinCardAnim:hide()
    self:pos(display.cx - MatchWinCardAnim.s_nodeW/2,display.height- (MatchWinCardAnim.s_nodeH/2)-18)
    transition.moveTo(self, {time = 2, x = display.width + (MatchWinCardAnim.s_nodeW/2)})
end

function MatchWinCardAnim:onEnter()
    -- 添加至舞台开始
    self:pos(display.width + MatchWinCardAnim.s_nodeW/2,display.height - (MatchWinCardAnim.s_nodeH/2)-18)
    transition.moveTo(self, {time = 2, x = display.width -(MatchWinCardAnim.s_nodeW/2)})
end

function MatchWinCardAnim:onExit()
    -- 从舞台移除停止动画
end

function MatchWinCardAnim:dispose()

end

return MatchWinCardAnim