...
local MatchHandPoker = class("MatchHandPoker")
function MatchHandPoker:ctor()
    self.cards_ = {}
    self.count_ = 0  -- 牌数量
    self.point_ = 0  -- 牌点数
    self.type_ = 1   -- 牌型
...
-- 牌型
function MatchHandPoker:getType()
    return self.type_
end

-- 获取牌型描述
function MatchHandPoker:getTypeLabel()
...
function MatchHandPoker:isPokdeng()
    return self.type_ == POKER_TYPE_ALL.POKDENG
end

function MatchHandPoker:isLargeCardType()
...
function MatchHandPoker:isGoodType()
    return self.type_ >= POKER_TYPE_ALL.STRAIGHT and self.type_ <= POKER_TYPE_ALL.POKDENG
end

function MatchHandPoker:isBadType()
    return self.type_ == POKER_TYPE_ALL.POINT_CARD
end

-- 是否需要第三张牌
function MatchHandPoker:needPoker()
    return self:isBadType() and self.point_ < 4 
end

-- 牌点
function MatchHandPoker:getPoint()
    return self.point_
end

-- 手牌
function MatchHandPoker:getCards()
    return self.cards_
end

-- 起手牌
function MatchHandPoker:setCards(cards)
...
--能否要第三张牌
function MatchHandPoker:canGetCard()
    return (not self:isPokdeng_()) and self.count_ == 2
end

-- 加第三张牌
function MatchHandPoker:addCard(card)
    self.cards_[3] = card   
    self:reset()
end


function MatchHandPoker:isPokdeng_()
...
function MatchHandPoker:isThreeKind_()
...
function MatchHandPoker:isStraightFlush_()
...
function MatchHandPoker:isStraight_()
    if self.count_ ~= 3 then
        return false
    end
...
function MatchHandPoker:isThreeYellow_()
    if self.count_ ~= 3 then
        return false
    end
    local val1 = getPokerValue(self.cards_[1])
...
function MatchHandPoker:isPointCard_()
...
function MatchHandPoker:compare(handPoker)   
