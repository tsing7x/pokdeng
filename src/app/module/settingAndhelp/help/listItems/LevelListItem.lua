...
function LevelListItem:foldContent()
    ...
    elseif self.index_ == 3 and not self.initItemOther then
            self.initItemOther = true
            self:createItem1(self.data_)
...
function LevelListItem:createItem3(data)
    ...
    for i = 1, #levelList do
        ...
        --称号
        local pokerTitleLabel = display.newTTFLabel({
                text = levelArray[2],
                size = size,
                color = color,
                align = ui.TEXT_ALIGN_LEFT
        ...
        --总经验
        local sumExpLabel = display.newTTFLabel({
                text = levelArray[3],
                size = size,
                color = color,
                align = ui.TEXT_ALIGN_LEFT
        ...
        --等级奖励
        local levelRewardLabel = display.newTTFLabel({
                text = levelArray[4],
                size = size,
                color = color,
                align = ui.TEXT_ALIGN_RIGHT
