...
function RuleListItem:foldContent()
    ...
    if self.index_  ~= 1 and self.index_ ~= 2 and not self.initItem then
        self.initItem = true
...
function RuleListItem:onDataSet(dataChanged, data)
    if dataChanged then
        ...
        if self.index_ == 1 then 
            --游戏规则图片
            local rulePic = display.newSprite("help_rule_pic.jpg"):addTo(self.bottomPanel_):scale(0.9)
            local padding = 10
            local h = rulePic:getContentSize().height - padding * 2

            self.bottomBackground_:size(self.contentWidth - 4, h - 30)
        ...
        elseif self.index_ == 2 then           
            local rulePic = display.newSprite("help_x_pic.jpg"):addTo(self.bottomPanel_):scale(0.9)
            local padding = 10
            local h = rulePic:getContentSize().height - padding * 2
