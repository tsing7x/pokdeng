...
function UpgradePopup:setupView()
    ...
    self.rewardLabel_ = display.newTTFLabel({
            text = "good boy",
            size = rewardTextSize, 
            color = whiteColor,
    ...
    self.btn_ = cc.ui.UIPushButton.new({normal = "#common_green_btn_up.png", pressed = "#common_green_btn_down.png"}, {scale9 = true})
        ...
        self.container_:addNodeEventListener(cc.NODE_TOUCH_EVENT, handler(self, self.onTouch_))
end

function UpgradePopup:onTouch_(event)
...
function UpgradePopup:onShareListener_()
    ...
    --      end
    --  end) 
    nk.sendFeed:up_grade_reward_(self.level_,self.rewards_,
        function()
            self:hide()
        end,
        function()
