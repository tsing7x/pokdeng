...
--
local GrabLampManager = class("GrabLampManager")
local RoomViewPosition = import(".views.GrabRoomViewPosition")
local log = bm.Logger.new("GrabLampManager")

function GrabLampManager:ctor()
end

function GrabLampManager:createNodes()
...
function GrabLampManager:show()
    self.lamp_:show()
end

function GrabLampManager:hide()
    self.lamp_:hide()
end

function GrabLampManager:getPositionId()
    return self.lampPostionId_ or 1
end

function GrabLampManager:turnTo(positionId, animation)
    ...
    local angleH = math.radian2angle(math.atan2((seatPosX - lampPosX) , (seatPosY - lampPosY)))
...
function GrabLampManager:setLampRotation(animation, rotation, scale)
...
function GrabLampManager:dispose()
end

return GrabLampManager