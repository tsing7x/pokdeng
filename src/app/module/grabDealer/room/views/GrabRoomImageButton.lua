...
--
local GrabRoomImageButton = {}

local function exportMethods(target)
    for k, v in pairs(GrabRoomImageButton) do
        if k ~= "create" then
            target[k] = v
        end
    end
...
function GrabRoomImageButton.create(filename)
    return exportMethods(display.newSprite(filename, x, y, params)):ctor()
end

function GrabRoomImageButton:ctor()
...
function GrabRoomImageButton:onClick(handler)
    self.clickHandler_ = handler
    return self
end

function GrabRoomImageButton:onHelperTouch_(evt, isTouchInSprite)
...
function GrabRoomImageButton:flash(times)
    times = times or 10
    if times == -1 then
...
function GrabRoomImageButton:stopFlash()
    self.isFlashing_ = false
    self:stopAllActions()
    self:setOpacity(128)
    return self
