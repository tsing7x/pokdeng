...
local GrabPotChipView = class("GrabPotChipView")
...
local logger = bm.Logger.new("GrabPotChipView")

function GrabPotChipView:ctor(parent, manager, potId)
    self.parent_ = parent
...
-- 创建筹码堆
function GrabPotChipView:createChipStack()
...
-- 重置筹码堆
function GrabPotChipView:resetChipStack(potChips)
...
function GrabPotChipView:moveToSeat(positionId, completeCallback)
...
function GrabPotChipView:moveToSeatComplete_()
...
function GrabPotChipView:reset()
    if self.potGrabChipData_ then
        self.manager_:recycleGrabChipData(self.potGrabChipData_)
        self.potGrabChipData_ = nil
    end
...
-- 清理
function GrabPotChipView:dispose()
    self:reset()
end

return GrabPotChipView