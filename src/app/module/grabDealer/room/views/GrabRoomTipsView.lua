...
--
local GrabRoomTipsView = class("GrabRoomTipsView", function() return display.newNode() end)

GrabRoomTipsView.WIDTH = display.width * 0.7 - 16

local TIPS = bm.LangUtil.getText("ROOM", "TIPS")
local INTERVAL = 30

function GrabRoomTipsView:ctor()
    self.background_ = display.newScale9Sprite("#room_tips_background.png", 0, 0, cc.size(GrabRoomTipsView.WIDTH, 72)):addTo(self)
    ...
    local w, h = GrabRoomTipsView.WIDTH - 4, 72 - 4
    ...
    self.label_ = display.newTTFLabel({size=24, color=cc.c3b(0x2b, 0x56, 0x86),
            ...
            dimensions=cc.size(GrabRoomTipsView.WIDTH - 8, 72),
            align=ui.TEXT_ALIGN_CENTER})
        :addTo(self.clip_)
end

function GrabRoomTipsView:play()
...
function GrabRoomTipsView:stop()
...
function GrabRoomTipsView:playNext_()
    if self.isPlaying_ then
...
function GrabRoomTipsView:randomIndexArr_(total)
    local src = {}
    for i = 1, total do
        src[i] = i
    end
