...
local GrabRaiseSlider = class("GrabRaiseSlider", function()
    return display.newNode()
end)

function GrabRaiseSlider:ctor()
    ...
    GrabRaiseSlider.THUMB_BOUND_TOP = trackY + 218 - 176 / 2
    GrabRaiseSlider.THUMB_BOUND_BOTTOM = trackY - 218 + 176 / 2
    GrabRaiseSlider.THUMB_BOUND_HEIGHT = GrabRaiseSlider.THUMB_BOUND_TOP - GrabRaiseSlider.THUMB_BOUND_BOTTOM
...
function GrabRaiseSlider:showPanel()
    self:setSliderPercentValue(0)
    return self:show()
end

function GrabRaiseSlider:setButtonStatus(allPotEnabled, q3PotEnabled, halfPotEnabled, tripleEnabled, isMaxAllin)
...
function GrabRaiseSlider:hidePanel()
    self:setSliderPercentValue(0)
    return self:hide()
end

function GrabRaiseSlider:onButtonClicked(callback)
    self.buttonClickedCallback_ = callback
    return self
end

function GrabRaiseSlider:setValueRange(valueMin, valueMax)
...
function GrabRaiseSlider:setValue(val)
...
function GrabRaiseSlider:getValue()
    return math.round(self:getSliderPercentValue() * self.valueRange_ + self.valueMin_)
end

function GrabRaiseSlider:setSliderPercentValue(newVal)
    assert(newVal >= 0 and newVal <= 1, "slider value must be between 0 and 1")
    self:onSliderPercentValueChanged_(newVal, true)
    self.thumb_:setPositionY(GrabRaiseSlider.THUMB_BOUND_BOTTOM + GrabRaiseSlider.THUMB_BOUND_HEIGHT * newVal)
    return self
end

function GrabRaiseSlider:getSliderPercentValue()
    return (self.thumb_:getPositionY() - GrabRaiseSlider.THUMB_BOUND_BOTTOM) / GrabRaiseSlider.THUMB_BOUND_HEIGHT
end

function GrabRaiseSlider:onSliderPercentValueChanged_(newVal, forceUpdate, needSound)
    if self.percentValue_ ~= newVal or forceUpdate then
        ...
        local prevTime = self.lastGrabRaiseSliderGearTickPlayTime_ or 0
        if needSound and self.prevValue_ ~= self.curValue_  and curTime - prevTime > 0.05 then
            self.lastGrabRaiseSliderGearTickPlayTime_ = curTime
...
function GrabRaiseSlider:onThumbTouch_(evt)
    ...
    elseif name == "moved" then
        ...
        if toY >= GrabRaiseSlider.THUMB_BOUND_TOP then
            toY = GrabRaiseSlider.THUMB_BOUND_TOP
        elseif toY <= GrabRaiseSlider.THUMB_BOUND_BOTTOM then
            toY = GrabRaiseSlider.THUMB_BOUND_BOTTOM
        end
        self.thumb_:setPositionY(toY)
        local val = (toY - GrabRaiseSlider.THUMB_BOUND_BOTTOM) / GrabRaiseSlider.THUMB_BOUND_HEIGHT
...
function GrabRaiseSlider:onButtonClicked_(tag)
