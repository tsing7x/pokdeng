...
local GrabSeatProgressTimer = class("GrabSeatProgressTimer",function()
    return display.newNode()
end)

function GrabSeatProgressTimer:ctor(countdown)
    self:setNodeEventEnabled(true)
    setCountdown(countdown - 1)
    self:addNodeEventListener(cc.NODE_ENTER_FRAME_EVENT, handler(self, self.onEnterFrame))
end

function GrabSeatProgressTimer:onEnter()
    -- 添加至舞台开始渲染
    self.frameCount_ = 0
...
function GrabSeatProgressTimer:onEnterFrame(evt, isFastForward)
...
function GrabSeatProgressTimer:remove()
    self:unscheduleUpdate()
    self:removeFromParent()
end

return GrabSeatProgressTimer