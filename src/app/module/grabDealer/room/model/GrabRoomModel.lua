...
local SeatStateMachine = import(".GrabSeatStateMachine")
local HandPoker = import(".GrabHandPoker")
local logger = bm.Logger.new("GrabRoomModel")
local GrabRoomModel = {}

function GrabRoomModel.new()
    local instance = {}
    local datapool = {}
    local function getData(table, key)
        return GrabRoomModel[key] or datapool[key]
    end
...
-- 是否是自己
function GrabRoomModel:isSelf(uid)
    return nk.userData.uid == uid
end

-- 是否正在游戏（游戏开始至游戏刷新，弃牌置为false）
function GrabRoomModel:isSelfInGame()
    return self.isSelfInGame_
end

-- 本人是否在座
function GrabRoomModel:isSelfInSeat()
    return self.selfSeatId_ >= 0 and self.selfSeatId_ <= 9
end

-- 本人是否有可参与牌局的牌
function GrabRoomModel:hasCardActive()
    return self.hasCardActive_
end

-- 本人是否为庄家
function GrabRoomModel:isSelfDealer()
    return self.selfSeatId_ == self.gameInfo.dealerSeatId
end


-- 获取自己的座位id
function GrabRoomModel:selfSeatId()
    return self.selfSeatId_
end

-- 获取自己
function GrabRoomModel:selfSeatData()
    return self.playerList[self.selfSeatId_]
end

-- 获取庄家
function GrabRoomModel:dealerSeatData()
    return self.playerList[self.gameInfo.dealerSeatId]
end

-- 获取当前房间类型
function GrabRoomModel:roomType()
    return self.roomType_
end
-- 获取庄家需要补币的额度
function GrabRoomModel:getDealerAddCoinNum()
    return self.dealerAddCoinNum_ or 0
end
--获取首先发牌的玩家座位号
function GrabRoomModel:getFirstDealCardSeatId()
    return self.firstDealCardSeatId_ or 0
end
--获取是否存在庄家
function GrabRoomModel:getHaveDealer()
    return self.haveDealer_ or 0
end
--获取抢庄最小门槛值
function GrabRoomModel:getGrabDealerNeedCoin()
...
--需要加上在牌局中赢的钱和减去输的钱)
function GrabRoomModel:getExchangeMoney()
    return self.exchangeMoney_ or 0
end
function GrabRoomModel:getCurBetMoney()
    return self.betMoeny_ or 0
end
-- 获取当前在桌人数
function GrabRoomModel:getNumInSeat()
    local num = 0
    for i = 0, 9 do
...
-- 获取牌桌所有用户的UID 
function GrabRoomModel:getTableAllUid()
    local tableAllUid = ""
    local userUid = ""
    local toUidArr = {}
    for seatId = 0, 9 do
...
function GrabRoomModel:getSeatIdByUid(uid)
    for seatId = 0, 9 do
        local player = self.playerList[seatId]
...
-- 获取本轮参与玩家人数
function GrabRoomModel:getNumInRound()
    local num = 0
    for i = 0, 9 do
...
function GrabRoomModel:getNewCardType(cardType, pointCount)
    return CardType.new(cardType, pointCount)
end


function GrabRoomModel:initWithLoginSuccessPack(pack)
    self.clearData()
    bm.DataProxy:setData(nk.dataKeys.ROOM_INFO_TYPE,2)
    ...
    roomInfo.gameType = consts.ROOM_TYPE.GRAB
    roomInfo.highBet = pack.highBet
    roomInfo.rmType = pack.rmType
...
function GrabRoomModel:processGameStart(pack)
    -- 设置gameInfo
...
function GrabRoomModel:isChangeDealer()
...
function GrabRoomModel:processBetSuccess(pack)
...
function GrabRoomModel:processPot(pack)
    self.gameInfo.totalAnte = pack.totalAnte  
end

--发牌
function GrabRoomModel:processDeal(pack)
...
--亮牌
function GrabRoomModel:processShowHand(pack)
    local player = self.playerList[pack.seatId] 
    player.cards = pack.cards
    player.isOutCard = 1
    player.cardsCount = #pack.cards
...
function GrabRoomModel:processTurnToGetPoker(pack)
...
function GrabRoomModel:processGetPoker(pack)
    local player = self.playerList[pack.seatId] 
...
function GrabRoomModel:processGetPokerBySelf(pack)
    local player = self:selfSeatData()     
    player.cardsCount = 3
...
function GrabRoomModel:processSitDown(pack)
    local player = pack
...
function GrabRoomModel:processStandUp(pack,isOther)
    local player = self.playerList[pack.seatId]
...
function GrabRoomModel:processGameOver(pack)
    self.turn_money_ = 0
...
function GrabRoomModel:processCardRecord(pack)
    local recordInfos = {}
...
function GrabRoomModel:processMaxWinMoney(winMoney)
...
--更新最大手牌
function GrabRoomModel:processBestCard(nowCards)
...
function GrabRoomModel:processSendChipSuccess(pack)
...
function GrabRoomModel:processSendExpression(pack)
    local player = self.playerList[pack.seatId]
...
function GrabRoomModel:processRoomBroadcast(pack)
...
function GrabRoomModel:processSvrAddvanceDealer(pack)
    self.dealerAddCoinNum_ = pack.addCoin
    return self.dealerAddCoinNum_
end
function GrabRoomModel:processGrabAdvance(pack)
    self.grabNeedCoin_ = pack.handCoin + 1
    if pack.handCoin < self.gameInfo.grabDoor then
        self.grabNeedCoin_ = self.gameInfo.grabDoor
    end
end
function GrabRoomModel:processDealerAddCoin(pack)
...
function GrabRoomModel:totalChipsInTable()
...
function GrabRoomModel:reset()  
    self.isSelfInGame_ = false
    self.betMoeny_ = 0
    self.hasCardActive_ = false
end

return GrabRoomModel