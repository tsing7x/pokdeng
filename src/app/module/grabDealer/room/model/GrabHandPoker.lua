...
local GrabHandPoker = class("GrabHandPoker")
function GrabHandPoker:ctor()
    self.cards_ = {}
    self.count_ = 0  -- 牌数量
    self.point_ = 0  -- 牌点数
    self.type_ = 1   -- 牌型
...
-- 牌型
function GrabHandPoker:getType()
    return self.type_
end

-- 获取牌型描述
function GrabHandPoker:getTypeLabel()
...
function GrabHandPoker:isPokdeng()
    return self.type_ == POKER_TYPE_ALL.POKDENG
end

function GrabHandPoker:isLargeCardType()
...
function GrabHandPoker:isGoodType()
    return self.type_ >= POKER_TYPE_ALL.STRAIGHT and self.type_ <= POKER_TYPE_ALL.POKDENG
end

function GrabHandPoker:isBadType()
    return self.type_ == POKER_TYPE_ALL.POINT_CARD
end

-- 是否需要第三张牌
function GrabHandPoker:needPoker()
    return self:isBadType() and self.point_ < 4 
end

-- 牌点
function GrabHandPoker:getPoint()
    return self.point_
end

-- 手牌
function GrabHandPoker:getCards()
    return self.cards_
end

-- 起手牌
function GrabHandPoker:setCards(cards)
...
--能否要第三张牌
function GrabHandPoker:canGetCard()
    return (not self:isPokdeng_()) and self.count_ == 2
end

-- 加第三张牌
function GrabHandPoker:addCard(card)
    self.cards_[3] = card   
    self:reset()
end


function GrabHandPoker:isPokdeng_()
...
function GrabHandPoker:isThreeKind_()
...
function GrabHandPoker:isStraightFlush_()
...
function GrabHandPoker:isStraight_()
    if self.count_ ~= 3 then
        return false
    end
...
function GrabHandPoker:isThreeYellow_()
    if self.count_ ~= 3 then
        return false
    end
    local val1 = getPokerValue(self.cards_[1])
...
function GrabHandPoker:isPointCard_()
...
function GrabHandPoker:compare(handPoker)   
