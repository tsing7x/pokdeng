...
function UserInfoOtherDialog:setAddFriendStatus()
    ...
    if self.data_ and nk.userData.friendUidList then
        ...
        local friendUserData = table.indexof(nk.userData.friendUidList, tostring(self.data_.uid))

        if not friendUserData then
            ...
            -- dump(friendData, "self:getFriendDataByUid.friendData :==================")
...
function UserInfoOtherDialog:setData(data)
    ...
    if data then
        ...
        self.reqGetUserInfoData_ = nk.http.getMemberInfo(data.userId or data.uid, function(retData)
            ...
            self.ranking_:setString("")


            self.cash_:setString(retData.match.point or 0)
    ...
    if self.ctx.model:isSelfInSeat() then
        ...
        if (nk.userData["aUser.money"] -  chips ) > roomData.minBuyIn then
           nk.http.sendCoinForRoomEx(nk.userData.uid,self.data_.uid, chips,room_type,self.ctx.model.roomInfo.blind,
                function (data)
                    -- dump(data, "nk.http.sendCoinForRoomEx.retData :====================")

                    bm.EventCenter:dispatchEvent({name = nk.eventNames.UPDATE_SEAT_ANTE_CHIP, data = {chip = -chips}})
                    self.data_.userInfo.money = self.data_.userInfo.money + chips
                    nk.server:sendCoinForRoomPlayer(self.data_.seatId, chips)
                    self:hide()
                end, function (errData)
                    dump(errData, "nk.http.sendCoinForRoomEx.errData :====================")
