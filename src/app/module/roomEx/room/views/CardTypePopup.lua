...
CardTypePopup.WIDTH = 460
CardTypePopup.HEIGHT = 551
...
function CardTypePopup:onRemovePopup(removeFunc)
    self:stopAllActions()
    transition.moveTo(self, {time=0.3, x=-CardTypePopup.WIDTH * 0.5, easing="OUT", onComplete=function() 
        removeFunc()
    end})
end
...
function CardTypePopup:onShowPopup()
    self:stopAllActions()
    transition.moveTo(self, {time=0.3, x=CardTypePopup.WIDTH * 0.5 + 8, easing="OUT", onComplete=function()
        if self.onShow then
            self:onShow()
        end
    end})
