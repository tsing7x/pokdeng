...
local RaiseSliderEx = class("RaiseSliderEx", function()
    return display.newNode()
end)

function RaiseSliderEx:ctor()
    ...
    RaiseSliderEx.THUMB_BOUND_TOP = trackY + 218 - 176 / 2
    RaiseSliderEx.THUMB_BOUND_BOTTOM = trackY - 218 + 176 / 2
    RaiseSliderEx.THUMB_BOUND_HEIGHT = RaiseSliderEx.THUMB_BOUND_TOP - RaiseSliderEx.THUMB_BOUND_BOTTOM
...
function RaiseSliderEx:showPanel()
    self:setSliderPercentValue(0)
    return self:show()
end

function RaiseSliderEx:setButtonStatus(allPotEnabled, q3PotEnabled, halfPotEnabled, tripleEnabled, isMaxAllin)
...
function RaiseSliderEx:hidePanel()
    self:setSliderPercentValue(0)
    return self:hide()
end

function RaiseSliderEx:onButtonClicked(callback)
    self.buttonClickedCallback_ = callback
    return self
end

function RaiseSliderEx:setValueRange(valueMin, valueMax)
...
function RaiseSliderEx:setValue(val)
...
function RaiseSliderEx:getValue()
    return math.round(self:getSliderPercentValue() * self.valueRange_ + self.valueMin_)
end

function RaiseSliderEx:setSliderPercentValue(newVal)
    assert(newVal >= 0 and newVal <= 1, "slider value must be between 0 and 1")
    self:onSliderPercentValueChanged_(newVal, true)
    self.thumb_:setPositionY(RaiseSliderEx.THUMB_BOUND_BOTTOM + RaiseSliderEx.THUMB_BOUND_HEIGHT * newVal)
    return self
end

function RaiseSliderEx:getSliderPercentValue()
    return (self.thumb_:getPositionY() - RaiseSliderEx.THUMB_BOUND_BOTTOM) / RaiseSliderEx.THUMB_BOUND_HEIGHT
end

function RaiseSliderEx:onSliderPercentValueChanged_(newVal, forceUpdate, needSound)
    if self.percentValue_ ~= newVal or forceUpdate then
        ...
        local prevTime = self.lastRaiseSliderExGearTickPlayTime_ or 0
        if needSound and self.prevValue_ ~= self.curValue_  and curTime - prevTime > 0.05 then
            self.lastRaiseSliderExGearTickPlayTime_ = curTime
...
function RaiseSliderEx:onThumbTouch_(evt)
    ...
    elseif name == "moved" then
        ...
        if toY >= RaiseSliderEx.THUMB_BOUND_TOP then
            toY = RaiseSliderEx.THUMB_BOUND_TOP
        elseif toY <= RaiseSliderEx.THUMB_BOUND_BOTTOM then
            toY = RaiseSliderEx.THUMB_BOUND_BOTTOM
        end
        self.thumb_:setPositionY(toY)
        local val = (toY - RaiseSliderEx.THUMB_BOUND_BOTTOM) / RaiseSliderEx.THUMB_BOUND_HEIGHT
...
function RaiseSliderEx:onButtonClicked_(tag)
