...
-- Description: AwardAddComfirmPopup.lua Create && Modified By TsingZhang.
--
...
local AddrComboItem = import("app.module.scoreMarket.ScoreComboItem")

local AwardAddComfirmPopup = class("AwardAddComfirmPopup", nk.ui.Panel)

function AwardAddComfirmPopup:ctor()
	-- body
	...
	self.controller_ = AwardAddController.new(self)
	self:renderMainView()
	self.controller_:getUserAddress(handler(self, self.bindAddressInfo_))
end

function AwardAddComfirmPopup:renderMainView()
	-- body
    ...
    self.combo_ = nk.ui.ComboboxView.new(params)
        ...
        :addTo(self.mainInfoEditBlockBgDent_, 100)
...
function AwardAddComfirmPopup:addAlertPopup(px, py, text)
	-- body
...
function AwardAddComfirmPopup:onNameEdit_(event)
	-- body
	if event == "began" then
...
function AwardAddComfirmPopup:onSexRdBtnSelChangeCallBack_(evt)
    -- body
...
function AwardAddComfirmPopup:onTelEdit_(event)
	-- body
	if event == "began" then
        -- 开始输入
    elseif event == "changed" then
...
function AwardAddComfirmPopup:onAddressEdit_(event)
	-- body
	if event == "began" then
        -- 开始输入
    elseif event == "changed" then
...
function AwardAddComfirmPopup:onEmailEdit_(event)
	-- body
	if event == "began" then
        -- 开始输入
    elseif event == "changed" then
...
function AwardAddComfirmPopup:onArgTermsChkBtnCallBack_(evt)
    -- body
...
function AwardAddComfirmPopup:onUpdateAddrInfoCallBack_()
    -- body
    self.updAddrInfoBtn_:setButtonEnabled(false)
...
function AwardAddComfirmPopup:bindAddressInfo_(data)
	-- body
	if data then
        ...
        -- if data.tel == nil or data.tel == "" or data.tel == " " then
        --     self.alertT_:show()
        -- end
    else
        -- self.alertT_:show()
...
function AwardAddComfirmPopup:isRightEmail(str)
...
function AwardAddComfirmPopup:showPanel(callBack)
	-- body
...
function AwardAddComfirmPopup:onShowed()
    -- body
    if self and self.combo_ then
        --todo
...
function AwardAddComfirmPopup:onEnter()
	-- body
end

function AwardAddComfirmPopup:onExit()
	-- body
...
function AwardAddComfirmPopup:onCleanup()
	-- body
