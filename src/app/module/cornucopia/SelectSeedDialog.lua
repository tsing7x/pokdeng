...
local SelectSeedDialog = class("SelectSeedDialog", Panel)
local DEFAULT_WIDTH = 540
local DEFAULT_HEIGHT = 360
local TOP_HEIGHT = 68
local PADDING = 32
local BTN_HEIGHT = 72
function SelectSeedDialog:ctor(parent,data)
    self.data_= data
    self.parent_ = parent
     cc.Director:getInstance():getTextureCache():removeUnusedTextures()
     self:setNodeEventEnabled(true)
     self:setTouchSwallowEnabled(false)
	 SelectSeedDialog.super.ctor(self, {540,360})
     self:addCloseBtn()
...
function SelectSeedDialog:show(id)
  self.tid = id;
	self:showPanel_()
end
function SelectSeedDialog:ingotSeedClick_()
...
function SelectSeedDialog:silverSeedClick_()
