...
function AboutPopup:ctor()
    ...
    --     AboutPopup.HEIGHT/2 - TOP_HEIGHT + TOP_HEIGHT/2, cc.size(AboutPopup.WIDTH, TOP_HEIGHT)):addTo(self)

    --顶部重复花纹
    -- display.newTilesSprite("repeat/panel_repeat_tex.png", cc.rect(-AboutPopup.WIDTH/2, -TOP_HEIGHT/2, AboutPopup.WIDTH, TOP_HEIGHT))
    --     :pos(-AboutPopup.WIDTH/2, AboutPopup.HEIGHT/2 - TOP_HEIGHT):addTo(self)
    ...
    --玩家uid
    local uidLabel = display.newTTFLabel({
    ...
    --粉丝页地址
    local fansAddrLabel = display.newTTFLabel({
    ...
    --服务条款
    local serviceLabel = display.newTTFLabel({
    ...
    --公司版权
    local rightLabel = display.newTTFLabel({
