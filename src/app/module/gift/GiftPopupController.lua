        ...
        if self.selfMyGiftData then
            ...
            if self.subSelectedTab_ == 1  then
                if #self.selfBuyGiftData == 0 then
                    ...
                    self:getGroupGiftData(self.selfBuyGiftData, nk.userData["aUser.gift"])
                end
            elseif self.subSelectedTab_ == 2 then
                if #self.friendPresentData == 0 then 
                    ...
                    self:getGroupGiftData(self.friendPresentData, nk.userData["aUser.gift"])
                end
    ...
    elseif self.mainSelectedTab_ == 2 then
        ...
        elseif selectedTab == 3 and self.systemPresentData then
            if #self.systemPresentData == 0 then
                ...
                self:getGroupGiftData(self.systemPresentData, nk.userData["aUser.gift"])
            end
        else
            self.view_:setNoDataTip(true)
            self.view_:setLoading(false)
...
function GiftPopupController:requestMyGiftData_()
    ...
    request = function()
        ...
        self.myGiftDataRequestId_ = nk.http.giftMineInfo("0,2,3,4",handler(self, self.onGetMyGiftData_),
        function()
        ...
        if self.selfMyGiftData and self.selfShopGiftData then
            for i=1,#self.selfShopGiftData do
                for j=1,#self.selfMyGiftData do
                    ...
                    elseif checkint(self.selfMyGiftData[j].pmode) == 0 and (checkint(self.selfMyGiftData[j].pnid) ==  checkint(self.selfShopGiftData[i].pnid)) then
                        self.selfMyGiftData[j].image = self.selfShopGiftData[i].image
                        self.selfMyGiftData[j].money = self.selfShopGiftData[i].money
                        self.selfMyGiftData[j].expire = self.selfMyGiftData[j].day
                        self.selfMyGiftData[j].giftType = 1
...
function GiftPopupController:useBuyGiftRequest(isRoom)
    if self.selectGiftId_ == nil or (self.selectGiftId_ == nk.userData["aUser.gift"]) or ( self.selectGiftId_ == 0) then
        return 
    end
   
    nk.http.useProps(self.selectGiftId_,function(data)
        -- local callBackBuyData =  json.decode(data)
        local callBackBuyData =  data
        -- if callBackBuyData.ret == 1 then
            nk.userData["aUser.gift"] = self.selectGiftId_
...
function GiftPopupController:requestPresentGiftData(isRoom)
    ...
    self.presentGiftRequestId_ = nk.http.giftBuy(self.selectGiftId_,self.uid_,function(data)
        if data then
            ...
            if isRoom then
                ...
                nk.server:sendRoomGift(self.selectGiftId_,{self.uid_})
            end
            bm.EventCenter:dispatchEvent({name = nk.eventNames.HIDE_GIFT_POPUP})
        end
...
function GiftPopupController:buyGiftRequest(isRoom)
    ...
    self.buyGiftRequestId_ = nk.http.giftBuy(self.selectGiftId_,nk.userData.uid,function(data)
        if data then
...
function GiftPopupController:requestPresentTableGift(isRoom)
    ...
    self.presentTableGiftRequestId_ = nk.http.giftBuy(self.selectGiftId_,self.tableUidArr_,function(data)
        if data then
            nk.TopTipManager:showTopTip(bm.LangUtil.getText("GIFT", "PRESENT_TABLE_GIFT_SUCCESS_TOP_TIP"))
            local sendType = 3
            if isRoom then
                nk.server:sendRoomGift(self.selectGiftId_, self.toUidArr_)
            end
            bm.EventCenter:dispatchEvent({name = nk.eventNames.HIDE_GIFT_POPUP})
        end
