...
function ScoreComboItem:ctor()
    ...
    local px = WIDTH*0.5;
    -- display.newScale9Sprite("#sm_line.png", px, 0, cc.size(WIDTH, 1)):addTo(self);
	...
	self.lbl_ = display.newTTFLabel({
			...
			-- dimensions=cc.size(WIDTH, HEIGHT)
		})
		:pos(px, 0)
		:addTo(self)
	-- 
	...
	self.btnGroup_:onButtonRelease(function(evt)
		-- nk.TopTipManager:showTopTip("::"..evt.x..":::"..self.btnPressedX_)
		-- print("::"..math.abs(evt.x - self.btnPressedX_))
...
function ScoreComboItem:selectHandler_(evt)
	-- nk.TopTipManager:showTopTip("self.data_::"..self.data_.title)
